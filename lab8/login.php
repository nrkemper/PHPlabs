<?php
  ...
  if($_POST){
    ...
    if(!isset($_SESSION['CurrentUser'])){ //if there is no current session for this user
      ...
      $successful_login=$database->ValidateLogin($g_username, $g_password);                    //will set the successful_login and print_inv_login flags appopriately
      $print_inv_login = !$successful_login; 
